...
public class SitemapScheduler implements Runnable {
    ...
    protected void activate(Configuration configuration) {
        generators = asSet(configuration.generators());
     ...
     * @return
     */
    private Set<String> getApplicableNames(Resource sitemapRoot) {
        Set<String> onDemandNames = generatorManager.getOnDemandNames(sitemapRoot);
        Set<String> toSchedule;
        if (generators == null) {
            // all names
            toSchedule = new HashSet<>(generatorManager.getNames(sitemapRoot));
            toSchedule.removeAll(onDemandNames);
        } else {
            // only those of the contained generators
            toSchedule = generatorManager.getGenerators(sitemapRoot).entrySet().stream()

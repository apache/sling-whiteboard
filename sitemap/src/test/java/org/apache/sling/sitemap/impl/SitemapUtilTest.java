...
public class SitemapUtilTest {
    ...
    public void testSingleRootSitemapName() {
        ...
        // when
        String unnamedSitemap = SitemapUtil.getSitemapSelector(root, root, SitemapGenerator.DEFAULT_SITEMAP);
    ...
    public void testMultiRootSitemapName() {
        ...
        String unnamedSecondLevelSitemap = SitemapUtil.getSitemapSelector(secondLevelRoot, topLevelRoot, SitemapGenerator.DEFAULT_SITEMAP);
        String namedSecondLevelSitemap = SitemapUtil.getSitemapSelector(secondLevelRoot, topLevelRoot, "foo");
        String unnamedThirdLevelSitemap = SitemapUtil.getSitemapSelector(thirdLevelRoot, topLevelRoot, SitemapGenerator.DEFAULT_SITEMAP);
    ...
    public void testSitemapResolutionFromFileName() {
        ...
        // when
        Map<Resource, String> invalid = SitemapUtil.resolveSitemapRoots(root, "foobar");
        Map<Resource, String> defaultSitemap = SitemapUtil.resolveSitemapRoots(root, "sitemap");
        Map<Resource, String> newsCandidates = SitemapUtil.resolveSitemapRoots(root, "news-sitemap");
        Map<Resource, String> productPageCandidates = SitemapUtil.resolveSitemapRoots(root, "products-product-page-tops-sitemap");
        Map<Resource, String> productCandidates = SitemapUtil.resolveSitemapRoots(root, "products-sitemap");

        // then
        assertThat(invalid, aMapWithSize(0));
        assertThat(defaultSitemap, aMapWithSize(1));
        assertThat(defaultSitemap.entrySet(), hasInOrder(anEntry(root, SitemapGenerator.DEFAULT_SITEMAP)));
        assertThat(newsCandidates, aMapWithSize(2));
        assertThat(newsCandidates.entrySet(), hasInOrder(
                anEntry(root, "news"),
                anEntry(news, SitemapGenerator.DEFAULT_SITEMAP))
        );
